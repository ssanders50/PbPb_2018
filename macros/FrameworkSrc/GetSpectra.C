TH1D * Framework::GetSpectra() {
  ...
  cout<<spname.data()<<endl;
  TH2D * ptcnt = (TH2D *) tf->Get(spname.data());
  //cout<<"tf: "<<tf->GetName()<<endl;
  
  //cout<<"GetSpectra: "<<spname.data()<<"\t"<<ptcnt<<endl;
  ...
  if(r[0].spec_NegEta == 0) {
    r[0].spec_NegEta = (TH1D *) spec->ProjectionX(Form("spec1d_%d_%d_NegEta",minCent,maxCent),ietamin1,ietamax1);
    r[0].spec_NegEta->SetMarkerStyle(20);
    r[0].spec_NegEta->SetMarkerColor(kBlue);
  } else {
    r[0].spec_NegEta->Add((TH1D *) spec->ProjectionX(Form("spec1d_%d_%d_NegEta",minCent,maxCent),ietamin1,ietamax1));
  }
  cout<<"ieta2 "<<ietamin2<<"\t"<<ietamax2<<endl;
  if(r[0].spec_PosEta == 0 ) {
    r[0].spec_PosEta = (TH1D *) spec->ProjectionX(Form("spec1d_%d_%d_PosEta",minCent,maxCent),ietamin2,ietamax2);
    r[0].spec_PosEta->SetMarkerStyle(25);
    r[0].spec_PosEta->SetMarkerColor(kRed);
  } else {
    r[0].spec_PosEta->Add((TH1D *) spec->ProjectionX(Form("spec1d_%d_%d_PosEta",minCent,maxCent),ietamin2,ietamax2));
  }
  cout<<"continue"<<endl;
//   TH1D * spec1d = (TH1D *) r[0].spec_NegEta->Clone(Form("spec1d_%d_%d",minCent,maxCent));
...
//   debug = false;
//   dirsave->cd();
//   spec1d->Scale(1./hntrk->Integral(hntrk->FindBin(minCent),hntrk->FindBin(maxCent)));
//   return spec1d;
  return 0l;
 }
