...
HiEvtPlaneFlatProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for (EvtPlaneCollection::const_iterator rp = evtPlanes_->begin();rp !=evtPlanes_->end(); rp++) {
	...
	if(useOffsetPsi_ && psiFlat>-4) {
	  double Q = sqrt(pow(soff,2)+pow(coff,2));
	  soff = Q*TMath::Sin(EPOrder[indx]*psiFlat);
	  coff = Q*TMath::Cos(EPOrder[indx]*psiFlat);
	}
	...
	//std::cout<<"HiEvtPlaneFlatProducer: "<<indx<<"\t"<<rp->angle(0)<<std::endl;
	++indx;
    }
