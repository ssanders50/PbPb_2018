  ...
  CentBinCompression_ ( iConfig.getParameter<int>("CentBinCompression") )
{
  ...
  if(useNtrk_) {
    ...
    //std::cout<<"useNtrk true"<<std::endl;
  }
...
EvtPlaneProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if (fabs(vtx) < minvtx_ || fabs(vtx) > maxvtx_) {
    return;
  }        
  //std::cout<<bin<<"\t"<<vtx<<std::endl;
    //calorimetry part
	...
	for(int i = 0; i<NumEPNames; i++) {
	  ...
	  if(EPDet[i]==HF) {
	    ...
	    if(loadDB_) w = tower_energyet*flat[i]->getEtScale(vtx,bin);
	    if(EPOrder[i]==1 ) {
	      if(MomConsWeight[i][0]=='y' && loadDB_ ) {
		w = flat[i]->getW(tower_energyet, vtx, bin);
	      }
	...
	for(int i = 0; i<NumEPNames; i++) {
	  ...
	  if(EPDet[i]==Tracker) {
	    ...
	    if(w>2.5) w=2.0;   //v2 starts decreasing above ~2.5 GeV/c
	      if(EPOrder[i]==1) {
		if(MomConsWeight[i][0]=='y' && loadDB_) {
		  w = flat[i]->getW(track_pt, vtx, bin);
		}
    ...
    for(int i = 0; i<NumEPNames; i++) {
      ...
      //std::cout<<"EvtPlaneProducer: "<<EPNames[i]<<"\t"<<ang<<std::endl;
    }
