...
class generalAndHiPixelTracks : public edm::stream::EDProducer<> {
  ...
  double ptcut_lowCent_;
  double ptcut_highCent_;
...
generalAndHiPixelTracks::generalAndHiPixelTracks(const edm::ParameterSet& iConfig):
 ...
 ptcut_lowCent_(iConfig.getParameter<double>("ptcut_lowCent")),
 ptcut_highCent_(iConfig.getParameter<double>("ptcut_highCent"))
{
...
generalAndHiPixelTracks::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  double ptCut = ptcut_highCent_;
  if (occ < 20) ptCut = ptcut_lowCent_;
