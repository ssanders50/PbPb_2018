...
for i in range(0, 5):
    ...
    print(dataset)
    runrange = str(iovs[i])+'_'+str(iovs[i+1]-1)
    print(runrange)
    reqname = 'PbPb2018_vn_general_'+runrange
    print(reqname)
    dirbase='/store/user/ssanders/PbPb2018_vn_MB_general_'+runrange
    print(dirbase)
    infiles=['json_DCSONLY_HI.txt','HeavyIonRPRcd_PbPb2018_general_'+runrange+'.db','offset_PbPb2018_general_'+runrange+'.root']
    print(infiles)
    parms=['noprint','lumifile=json_DCSONLY_HI.txt','offset=offset_PbPb2018_general_'+runrange+'.root','dbfile=HeavyIonRPRcd_PbPb2018_general_'+runrange+'.db']
    print(parms)
